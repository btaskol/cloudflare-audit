...
# Credenciales y configuración desde variables de entorno / GitHub Secrets
...
if not API_TOKEN or not ACCOUNT_ID or not AI_API_KEY:
    print("[!] Error: Faltan credenciales necesarias en los secrets de Cloudflare o la IA.")
    sys.exit(1)
...
def audit_code_with_ai(code_content, access_policies):
    ...
    1. NO reportes "ausencia de autenticación" ni fallos de control de acceso, ya que la autenticación de usuarios se gestiona de manera perimetral a través de Cloudflare Access (ver políticas arriba).
...
def send_email_report(report_content):
    if not GMAIL_USER or not GMAIL_APP_PASSWORD:
        print("[!] Aviso: Credenciales de Gmail no configuradas, omitiendo envío de correo.")
        return

    print("[*] Enviando reporte por correo electrónico...")
    msg = MIMEMultipart()
    msg['From'] = GMAIL_USER
    msg['To'] = GMAIL_USER
...
def save_and_push_report(report_content):
    print("[*] Guardando y subiendo reporte al repositorio...")
    ...
    try:
        ...
        if status.stdout.strip():
            ...
            print("[OK 🟢] Reporte actualizado y pusheado en el repositorio.")
        else:
            print("[*] No hay cambios nuevos en el reporte respecto al anterior.")
